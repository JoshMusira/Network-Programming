...
const QueryAudio = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    websocket.onclose = (event) => {
      setIsLoading(false);
      setDone(true); // Set 'done' state to true when WebSocket closes
    };
  };
  ...
  return (
    <div className=" flex items-center  flex-col justify-center py-3 px-4 sm:px-6 lg:px-8">
      <div className="fex  w-full shadow-sm shadow-white rounded-lg p-2 justify-around items-start">
        <label className="flex  text-xl items-start font-bold text-blue-500">
          Select Database:
        </label>
        <div className="mt-1  border-b-white flex  w-full justify-around pb-2  space-y-2">
          <div className="flex items-center">
            ...
            />
            <label
              htmlFor="sports_local"
              className="ml-3 block text-sm font-semibold text-white"
            >
              Sports Database
            </label>
          </div>
      ...
      {done && <p className="text-green-400 text-xl">Done</p>}
    </div>
  );
};

export default QueryAudio;
